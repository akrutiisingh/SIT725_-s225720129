...
function makeValidBook(id) {
  return {
    ...
    year: 2020,
    genre: "Fiction",
    summary: "A valid summary that satisfies your rules.",
    price: "19.99"
  };
}
...
function makeValidUpdate() {
  return {
    ...
    year: 2021,
    genre: "Fantasy",
    summary: "Updated summary for testing update validation.",
    price: "25.50"
  };
}
...
await test({
  ...
  path: createPath,
  expected: 400,
  body: { ...makeValidBook(`b${Date.now()+3}`), year: "invalid" },
...
await test({
  ...
  path: createPath,
  expected: 400,
  body: { ...makeValidBook(`b${Date.now()+4}`), price: "-5" },
